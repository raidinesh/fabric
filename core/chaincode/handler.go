...
func (handler *Handler) triggerNextStateSync(msg *pb.ChaincodeMessage) {
	...
	if err = handler.serialSend(msg); err != nil {
		fmt.Errorf("[%s]Error sending   message, ending stream: %s", shorttxid(msg.Txid), err)
		handler.errc <- err
	}
}
...
func (handler *Handler) beforeRegisterEvent(msg *pb.ChaincodeMessage, state string) {
	...
	err := proto.Unmarshal(msg.Payload, chaincodeID)
	if err != nil {
		fmt.Errorf("Error in received %s, could NOT unmarshal registration info: %s", pb.ChaincodeMessage_REGISTER, err)
		return
	}
	...
	if err := handler.serialSend(&pb.ChaincodeMessage{Type: pb.ChaincodeMessage_REGISTERED}); err != nil {
		fmt.Errorf("Error sending %s: %s", pb.ChaincodeMessage_REGISTERED, err)
		handler.notifyDuringStartup(false)
		return
	}
...
func (handler *Handler) HandleMessage(msg *pb.ChaincodeMessage) error {
	...
	//return filteredErre
	panic("Check whey code is comming here")
	return nil
}
