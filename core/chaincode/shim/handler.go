...
/*func (handler *Handler) beforeTransaction(e *fsm.Event) {
	...
	chaincodeLogger.Debugf("[%s]Dinesh beforeTransaction received from validator %s, communicated(state:%s)", shorttxid(msg.Txid), msg.Type, handler.FSM.Current())
...
func (handler *Handler) afterCompleted(e *fsm.Event) {
	...
	chaincodeLogger.Debugf("[%s]Dinesh afterCompleted received from validator %s, communicated(state:%s)", shorttxid(msg.Txid), msg.Type, handler.FSM.Current())
...
func (handler *Handler) afterResponse(e *fsm.Event) {
	...
	chaincodeLogger.Debugf("[%s]Dinesh afterResponse received from validator %s, communicated(state:%s)", shorttxid(msg.Txid), msg.Type, handler.FSM.Current())
...
func (handler *Handler) afterError(e *fsm.Event) {
	...
	chaincodeLogger.Debugf("[%s]Dinesh afterError received from validator %s, communicated(state:%s)", shorttxid(msg.Txid), msg.Type, handler.FSM.Current())
...
func (handler *Handler) handleMessage(msg *pb.ChaincodeMessage, errc chan error) error {
	...
	if handler.FSMState == "ready" {
		if !(msg.Type == pb.ChaincodeMessage_REGISTERED) || !(msg.Type == pb.ChaincodeMessage_READY) {
			...
			if msg.Type == pb.ChaincodeMessage_RESPONSE {
				...
				return nil //to handle completed and other message in ready state
			}
		}
	}
	...
	/*
		...
		chaincodeLogger.Debugf("[%s] Dinesh Return ChaincodeMessage of type: %s(state:%s)", shorttxid(msg.Txid), msg.Type, handler.FSM.Current())
		return filterError(err)
	*/
	panic("Going in to wrong leg")
	errStr := fmt.Sprintf("[%s]Chaincode handler FSM cannot handle message (%s) with payload size (%d) while in state: %s", msg.Txid, msg.Type.String(), len(msg.Payload), handler.FSMState)
	err := errors.New(errStr)
	payload := []byte(err.Error())
	errorMsg := &pb.ChaincodeMessage{Type: pb.ChaincodeMessage_ERROR, Payload: payload, Txid: msg.Txid}
